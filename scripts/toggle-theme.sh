...
if [[ $current_state == "dark" ]]; then
    ...
    ln -sf ~/.local/share/themes/alacritty/modus-operandi.toml ~/.config/alacritty/current_theme.toml
else
    echo "dark" > ~/.local/state/theme
    emacsclient -e "(load-theme 'modus-vivendi)"
    gsettings set org.gnome.desktop.interface color-scheme "prefer-dark"
    gsettings set org.gnome.desktop.interface gtk-theme "Adwaita-dark"
    ln -sf ~/.local/share/themes/alacritty/modus-vivendi.toml ~/.config/alacritty/current_theme.toml
fi
